...
describe("unixShell", () => {
  ...
  // POSIX hosts only: there is no /bin/zsh to find on Windows, and run_command
  // never asks for one there (shellInvocation goes to cmd.exe or Git Bash).
  it.skipIf(process.platform === "win32")(
    "picks a shell that exists, preferring zsh",
    () => {
      ...
      expect(shell.login).toBe(
        shell.path.endsWith("zsh") || shell.path.endsWith("bash"),
      );
    },
  );
...
describe("resolveLoginPath", () => {
  ...
  // Same reason: the darwin half of this probes a login shell, which a Windows
  // host has none of. The win32 half is covered by the case above it.
  it.skipIf(process.platform === "win32")(
    "caches per platform, so the parameter keeps working after the first call",
    async () => {
...
describe("execEnv", () => {
  ...
  it("writes PATH under one casing only", () => {
    ...
    expect(Object.keys(win).filter((k) => k.toUpperCase() === "PATH")).toEqual([
      "Path",
    ]);
    const posix = execEnv("/usr/bin", "darwin");
    expect(
      Object.keys(posix).filter((k) => k.toUpperCase() === "PATH"),
    ).toEqual(["PATH"]);
  });
});
