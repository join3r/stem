...
describe('scheduled-run model restore', () => {
  ...
  it('never captures a persona-authored mail delivery as the user', async () => {
    // A mail delivery's prompt is the delivered mail body. A driver's directive
    // to a spoke (or a spoke's report back) arrived here as ordinary input and
    // was queued for user-role episodic capture — so the distiller later cited
    // it as the user's own words ("The user wants …", 0.9 confidence). Only
    // mail the user actually wrote may be captured as the user.
    const currentTurn = (runtime: PiRuntime) =>
      (runtime as unknown as { workers: FakeWorker[] }).workers[0]?.currentTurn;

    const personaMail = await scheduledRuntime();
    await personaMail.runtime.startTurn({
      input: 'Please make truncation Unicode-safe and add a regression test.',
      threadId: 'sched-1',
      mail: { conversationId: 'conv-1', subject: 'subjects', from: 'normal', participants: ['normal', 'spoke'] }
    });
    expect(currentTurn(personaMail.runtime)?.pendingUserCapture).toBeUndefined();

    // The user's own mail keeps its capture: it really is the user speaking.
    const userMail = await scheduledRuntime();
    await userMail.runtime.startTurn({
      input: 'I want mail subjects autogenerated when missing.',
      threadId: 'sched-1',
      mail: { conversationId: 'conv-1', subject: 'subjects', from: 'user', participants: ['normal', 'spoke'] }
    });
    expect(currentTurn(userMail.runtime)?.pendingUserCapture).toMatchObject({
