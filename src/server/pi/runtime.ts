...
const { getTurnActivitiesByThread, getTurnTimingsByThread, getTurnSystemsByThread, upsertTurnActivity, upsertTurnTiming, upsertTurnSystem, setActiveFacts, recordTurnInjectedFacts, recordTurnInjectedDocs } = recallStore;
...
export class PiRuntime extends EventEmitter implements ChatBackend {
  ...
  private async recordTurnEntry(worker: PiWorker, turn: TurnContext): Promise<void> {
    try {
      ...
      // Which version of the persona / skills / memory code ran this turn — the
