...
- Do NOT record response-STYLE directives (how long or short replies should be, tone, output format, language style, whether to use components). Those are managed separately as the user's custom instructions, NOT as facts — leave them out entirely.
- DO record the user's standing preferences about how a kind of TASK should be done for them ("wants absence summaries to include the #war-room channel and any downtime", "wants flight searches to prefer window seats", "wants work summaries to cover email as well as Slack"). These are durable preference facts, not style rules — a stated correction like "you didn't check X; I want that included in the future" is exactly such a fact.
