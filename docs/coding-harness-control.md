...
| 11 | Mail deliveries: code personas only (2026-09-01). A persona's harness pin is the capability, not a default — a mail turn whose persona has no pin gets `coding_agent` refused (the turn-context gate's `coding` field saves the round-trip; the bridge in `runtime.ts` enforces it), and a pinned persona's calls are CLAMPED to the pin: agent and device from the pin verbatim, cwd only the pinned folder or inside it (`harness/pin.ts`). Interactive chats are unchanged. An agent-created or agent-edited persona can never gain a pin (`savePersonaFor` drops it), so coding powers are editor-granted only. |
