...
| 4 | Escalations run exec's approval tiers BEFORE any card (2026-08-22, superseding the pure pass-through): `HarnessService.askPermission` answers a kind-'execute' ask itself when `exec.approvalMode` clears it — yolo allows everything, the shared allowlist (device bucket for device hosts) clears tier 1, the shared LLM judge (exec/judge.ts, intent = the harness brief) clears tier 2 — always via the adapter's `allow_once` option, never `allow_always` (that would teach the AGENT a permanent rule nobody saw). A command referencing a read-only root is NEVER auto-approved, any mode; its card carries the guard's reason. Only escalations become the fifth approval card (`ApprovalCard`, shared with exec since 2026-09-04: it renders INLINE above the composer of the chat that asked, with a notice bar and an amber sidebar dot in every other chat, and stays a modal only where no chat can hold it — Quick Chat for another thread's ask), annotated with `judgeVerdict`/`judgeReason`/`guardReason`; the buttons say the decision (`Reject` / `Always allow` / `Allow`, from the option `kind`) and the rule an `allow_always` would teach is a line beside them, options otherwise the harness's own and diff content rendered when the ask carries one. Exec's two invariants copied: the 10 minute clock arms only when the card is visible, and expiry is reported as "nobody answered", never as a refusal. Cards replay on the connect snapshot and push to the phone (`ApprovalPushKind` 'harness'). Auto-decisions leave a `log('harness', 'auto-approved...')` line — the only record of what ran without a card. |
...
| 6 | ~~The settings switch is OFF by default (`Settings -> Chat -> Coding agents`), read fresh per request.~~ Superseded by 14: there is no Stem-wide switch. Device runs are gated by a client-local consent switch ("Run coding agents on this computer", `harness-host.json`, 0600, never on the wire), read fresh on every request by the machine that would run the agent. |
...
| 11 | Mail deliveries: code personas only (2026-09-01; widened to every turn kind by 14). A persona's harness pin is the capability, not a default — a mail turn whose persona has no pin gets `coding_agent` refused (the turn-context gate's `coding` field saves the round-trip; the bridge in `runtime.ts` enforces it), and a pinned persona's calls are CLAMPED to the pin: agent and device from the pin verbatim, cwd only the pinned folder or inside it (`harness/pin.ts`). ~~Interactive chats are unchanged.~~ An agent-created or agent-edited persona can never gain a pin (`savePersonaFor` drops it), so coding powers are editor-granted only. |
| 12 | Offline hold (2026-09-01): a user mail to a persona pinned to a paired computer that cannot run coding agents right now (disconnected, or its consent switch off) is HELD, not delivered — the user gets a notice mail naming the machine, the wait persists in `mail-device-queue.json`, and the mail delivers automatically when that device's `harnessHost:announce` lands (clients re-announce on every reconnect; a boot sweep covers a device already online). Stop drops the wait. Fan-out branches and reply hops never hold — a join must not wedge on a sleeping laptop; their turns run and `coding_agent` reports the offline device itself. |
| 13 | The MODEL a coding agent runs is part of the persona's pin (`PersonaHarnessPin.model`, 2026-09-04), not a server-wide setting: a persona is the unit of run configuration, so two personas can drive Claude Code on different models. The persona editor lists what the agent advertises ON THE PINNED HOST (a `models` op on the device wire — `acpx getStatus().models` — the server's own acpx for unpinned personas); empty = the agent's own default on that machine. The pin rides every ensure and turn as `sessionOptions.model` because acpx hides the machine's `~/.claude` config from Stem-run sessions. A stored `harness.agents.<agent>.model` (the short-lived global pin) is dropped on settings read. |
| 14 | Code personas only, everywhere (2026-09-04). Decision 11's pin-as-capability now covers interactive chats and scheduled runs too: a turn that runs as no persona with a harness pin has no `coding_agent` — the turn-context gate's `coding` field is `!!personaHarness`, the bridge refuses without a pin and clamps with one, in every turn kind. The tool lost its `agent` and `device` parameters (the pin fixes them; `cwd` may only name a folder inside the pinned one). The Stem-wide "Delegate coding work" switch (`harness.enabled`) is gone from settings, desktop and phone — a stored value is dropped on read — and the base system prompt no longer pitches delegation; the brief that does is appended to a CODE persona's role prompt (`codingDelegationInstructions`, spawn-time, so editing the pin re-spawns the worker like editing the prompt). Why: with the global switch on, every plain chat had the tool AND a standing instruction to use it for "real software work", so "build the iOS app" launched Claude Code from a chat with no persona, no folder and no model pin. The client-local "Run coding agents on this computer" consent is unchanged: it is a device's consent, not a chat's capability. |
