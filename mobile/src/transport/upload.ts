...
export async function uploadAttachment(pairing: StoredPairing, attachment: DraftAttachment): Promise<TurnAttachment> {
  ...
  const prepared = await prepareImageUpload(attachment, await file.bytes());
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 120_000);
  try {
    const response = await expoFetch(`${pairing.serverUrl}/upload?name=${encodeURIComponent(prepared.name)}`, {
      method: 'POST', headers: { Authorization: `Bearer ${pairing.token}`, 'Content-Type': prepared.mime ?? 'application/octet-stream' },
      ...
      body: prepared.bytes, signal: controller.signal, redirect: 'error'
    });
    ...
    return {name: prepared.name, mime: prepared.mime, path: result.result.handle};
